...
function forward_transform() {
    ...
    for (let ih = 0; ih < idCanvas.height; ++ih) {
        ...
        dwt_forward(imR, ih * imageData.width, imageData.width, 0);
        dwt_forward(imR, ih * imageData.width, imageData.width, 1);
        dwt_forward(imG, ih * imageData.width, imageData.width, 0);
        dwt_forward(imG, ih * imageData.width, imageData.width, 1);
        dwt_forward(imB, ih * imageData.width, imageData.width, 0);
        dwt_forward(imB, ih * imageData.width, imageData.width, 1);
        for (let iw = 0; iw < idCanvas.width; ++iw) {
...
function inverse_transform() {
    ...
    //imageData = ctx.getImageData(0, 0, img.naturalWidth, img.naturalHeight);
    for (let ih = 0; ih < idCanvas.height; ++ih) {
        dwt_inverse(imR, ih * imageData.width, imageData.width, 1);
        dwt_inverse(imR, ih * imageData.width, imageData.width, 0);
        dwt_inverse(imG, ih * imageData.width, imageData.width, 1);
        dwt_inverse(imG, ih * imageData.width, imageData.width, 0);
        dwt_inverse(imB, ih * imageData.width, imageData.width, 1);
        dwt_inverse(imB, ih * imageData.width, imageData.width, 0);
        for (let iw = 0; iw < idCanvas.width; ++iw) {
...
function dwt_forward(im, beg, len, level) {
    const inc = 1 << level;
	const end = beg + len;
...
function dwt_inverse(im, beg, len, level) {
    const inc =  1 << level;
	const end = beg + len;
    //assert(inc < end && "stepping outside source image");
